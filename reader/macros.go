...
func UnmatchedDelimiter() Macro {
	return func(_ *Reader, initRune rune) (value.Any, error) {
		return nil, UnmatchedDelimiterError(initRune)
	}
}
...
func readNumber(rd *Reader, init rune) (value.Any, error) {
	...
	case isRadix && (decimalPoint || isScientific):
		return nil, NumberFormatError(numStr)
...
func readString(rd *Reader, _ rune) (value.Any, error) {
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				return nil, ReadError{FormType: "string", Value: ErrEOF}
			}
...
func readCharacter(rd *Reader, _ rune) (value.Any, error) {
	r, err := rd.NextRune()
	if err != nil {
		return nil, ReadError{FormType: "character", Value: ErrEOF}
	}
...
func quoteFormReader(expandFunc string) Macro {
	return func(rd *Reader, _ rune) (value.Any, error) {
		...
		if err != nil {
			if err == io.EOF {
				return nil, ReadError{FormType: expandFunc, Value: ErrEOF}
			} else if err == ErrSkip {
				return nil, ReadError{
					FormType: expandFunc,
					Value:    errors.New("cannot quote a no-op form"),
				}
			}
			return nil, err
		}
