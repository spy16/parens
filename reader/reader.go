...
func (rd Reader) Container(end rune, formType string, f func(value.Any) error) error {
	for {
		if err := rd.SkipSpaces(); err != nil {
			if err == io.EOF {
				return ReadError{FormType: formType, Value: ErrEOF}
			}
			return err
		}
...
// Path satisfies parens.PositionProvider
func (p Position) Path() string { return p.File }

// Line satisfies parens.PositionProvider
func (p Position) Line() int { return p.Ln }

// Column satisfies parens.PositionProvider
